...
class SideBar extends Widget
{
    ...
    public function init()
    {
        foreach ($this->items as $key => $item) {
            $this->_putActives($route = \Yii::$app->controller->route, $item, $key, []);
        }
    ...
    private function _putActives($route, $current, $currentKey, $keys = [])
    {
        if (isset($current['items']) && $current['items']) {
            $keys[] = $currentKey;
            foreach ($current['items'] as $key => $item) {
                $this->_putActives($route, $item, $key, $keys);
            }
        } else {
            if (strpos($route, trim($current['url'][0], '/')) === 0) {
                $keys[] = $currentKey;
                $this->actives = $keys;
            }
        }
